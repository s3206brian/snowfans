...
import { updateTrip, type TripState } from '@/app/actions/trips'
...
export function EditTripForm({ trip }: { trip: Trip }) {
  const [state, action, isPending] = useActionState(updateTrip.bind(null, trip.id), initialState)
          ...
          {STATUS_OPTIONS.map(({ value, label }) => (
            <label key={value} className="flex items-center gap-3 cursor-pointer group">
              <input
                ...
                value={value}
                required
                defaultChecked={trip.status === value}
